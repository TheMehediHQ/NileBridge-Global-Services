...
<div class="relative min-h-screen bg-[#F8FAFC] text-slate-800 py-12 px-4 sm:px-6 lg:px-8 flex items-center justify-center overflow-hidden">
    ...
    <!-- Ambient Atmospheric Gradients & Texture (Matching NileBridge System) -->
    ...
    <div class="max-w-md w-full space-y-4 relative z-10">
        ...
        <div class="flex items-center justify-between px-1">
            ...
            <span class="text-[11px] font-mono text-slate-400">Security &amp; Recovery</span>
        </div>
        ...
        <div class="w-full space-y-6 bg-white border border-slate-200/90 p-8 sm:p-10 rounded-3xl shadow-[0_8px_30px_rgba(11,21,47,0.06)]">
            ...
            <div class="text-center">
                ...
                <h2 class="text-2xl sm:text-3xl font-extrabold text-[#0B152F] tracking-tight">Reset Password</h2>
                <p class="mt-2 text-xs sm:text-sm text-slate-500 leading-relaxed">
                    Provide your enterprise corporate email and we will dispatch a secure, time-limited cryptographic reset link.
                </p>
            </div>

            <!-- Session Status Feedback Alert -->
            @if (session('status'))
            ...
            <!-- Form -->
            <form class="mt-6 space-y-5" action="{{ route('password.email') }}" method="POST">
                @csrf
            ...
            <div class="text-center text-[11px] text-slate-400 leading-relaxed">
                Tokens expire in 60 minutes. Protected under NileBridge Enterprise Security Standards.
            </div>
        </div>
    </div>
</div>
